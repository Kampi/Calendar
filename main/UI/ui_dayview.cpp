...
lv_obj_t *UI_DayView_Create(void)
{
    ...
    lv_obj_clear_flag(dayview_screen, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scrollbar_mode(dayview_screen, LV_SCROLLBAR_MODE_OFF);
    ...
    lv_obj_set_style_text_font(header_label, &lv_font_montserrat_ext_28, 0);
    ...
    lv_obj_clear_flag(event_list, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_scrollbar_mode(event_list, LV_SCROLLBAR_MODE_OFF);
                          ...
                          const char *Location, const char *Description)
{
    ...
    /* Calculate dynamic box height based on location line count */
    int LocationLineCount = 1;
    if ((Location != NULL) && (strlen(Location) > 0)) {
        const char *p_Char = Location;
        while (*p_Char) {
            if (*p_Char == '\n') {
                LocationLineCount++;
            }
    ...
    int BoxHeight = UI_EVENT_BOX_HEIGHT + ((LocationLineCount > 1) ? (LocationLineCount - 1) * 24 : 0);

    /* Create event box */
    lv_obj_t *event_box = lv_obj_create(event_list);
    lv_obj_set_size(event_box, LV_PCT(98), BoxHeight);
    ...
    lv_obj_set_scrollbar_mode(event_box, LV_SCROLLBAR_MODE_OFF);
    ...
    lv_obj_remove_style_all(details_container);
    ...
    lv_obj_set_style_text_font(title_label, &lv_font_montserrat_ext_20, 0);
    ...
    lv_obj_set_style_text_font(location_label, &lv_font_montserrat_ext_20, 0);
    ...
    lv_obj_set_style_text_font(desc_label, &lv_font_montserrat_ext_20, 0);
...
void UI_DayView_Clear_Events(void)
{
    if (event_list != NULL) {
        ...
        lv_obj_set_style_text_font(no_events_label, &lv_font_montserrat_ext_40, 0);
