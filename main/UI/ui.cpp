...
#define UI_HOUR_HEIGHT              40
#define UI_DAY_WIDTH                215
#define UI_TIME_LABEL_WIDTH         50
...
static lv_obj_t *day_labels[2] = {NULL, NULL};
...
static void show_event_info(Event_Data_t *p_EventData)
{
    if (info_modal != NULL) {
        lv_obj_del(info_modal);
        info_modal = NULL;
    ...
    /* Create info box */
    lv_obj_t *info_box = lv_obj_create(info_modal);
    ...
    lv_obj_t *content = lv_obj_create(info_box);
    ...
    if (p_EventData->start_time || p_EventData->end_time) {
        ...
        lv_obj_t *time_container = lv_obj_create(content);
        lv_obj_set_size(time_container, lv_pct(100), LV_SIZE_CONTENT);
        lv_obj_set_style_bg_color(time_container, lv_color_hex(0xF0F0F0), 0);
        ...
        lv_obj_t *time_icon = lv_label_create(time_container);
        // TODO
    ...
    if (p_EventData->location && (strlen(p_EventData->location) > 0)) {
        lv_obj_t *loc_container = lv_obj_create(content);
        ...
        lv_obj_t *loc_icon = lv_label_create(loc_container);
        ...
        lv_obj_t *loc_label = lv_label_create(loc_container);
    ...
    if (p_EventData->description && strlen(p_EventData->description) > 0) {
        lv_obj_t *desc_container = lv_obj_create(content);
        ...
        lv_obj_t *desc_icon = lv_label_create(desc_container);
        ...
        lv_obj_t *desc_label = lv_label_create(desc_container);
    ...
    /* Close button */
    lv_obj_t *close_btn = lv_btn_create(info_box);
...
void UI_Dashboard_Create(uint8_t Hours_Start, uint8_t Hours_End)
{
    ...
    lv_obj_set_style_bg_color(dashboard_screen, lv_color_hex(0xE0E0E0), 0);

    /* Create header container */
    lv_obj_t *header = lv_obj_create(dashboard_screen);
    lv_obj_set_size(header, LV_HOR_RES, UI_HEADER_HEIGHT);
    lv_obj_set_pos(header, 0, 0);
    lv_obj_set_style_bg_color(header, lv_color_hex(0xC0C0C0), 0);
    ...
    lv_label_set_text(header_label, "Dashboard - Loading...");
    ...
    lv_obj_set_size(calendar_container, LV_HOR_RES - 20, LV_VER_RES - UI_HEADER_HEIGHT - 30);
    lv_obj_set_pos(calendar_container, 10, UI_HEADER_HEIGHT + 10);
    lv_obj_set_style_bg_color(calendar_container, lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_border_width(calendar_container, 2, 0);
    lv_obj_set_style_border_color(calendar_container, lv_color_hex(0x808080), 0);
    lv_obj_set_style_radius(calendar_container, 5, 0);
    lv_obj_set_style_pad_all(calendar_container, 10, 0);

    /* Create time column labels (07:00 - 21:00) */
    for (uint8_t i = 0; i <= (_ui_Hours_End - _ui_Hours_Start); i++) {
        lv_obj_t *time_label = lv_label_create(calendar_container);
        char time_str[10];
        snprintf(time_str, sizeof(time_str), "%02d:00", _ui_Hours_Start + i);
        lv_label_set_text(time_label, time_str);
        lv_obj_set_style_text_font(time_label, &lv_font_montserrat_12, 0);
        lv_obj_set_style_text_color(time_label, lv_color_hex(0x000000), 0);
        lv_obj_set_pos(time_label, 5, 25 + i * UI_HOUR_HEIGHT);
    }

    /* Create vertical line after time labels */
    lv_obj_t *time_line = lv_obj_create(calendar_container);
    lv_obj_set_size(time_line, 2, (_ui_Hours_End - _ui_Hours_Start + 1) * UI_HOUR_HEIGHT + 10);
    lv_obj_set_pos(time_line, UI_TIME_LABEL_WIDTH + 5, 20);
    lv_obj_set_style_bg_color(time_line, lv_color_hex(0x808080), 0);
    lv_obj_set_style_border_width(time_line, 0, 0);
    lv_obj_clear_flag(time_line, LV_OBJ_FLAG_SCROLLABLE);
    ...
    for (uint8_t i = 0; i < 2; i++) {
        ...
        if (i < 1) {
            lv_obj_t *day_line = lv_obj_create(calendar_container);
            lv_obj_set_size(day_line, 2, (_ui_Hours_End - _ui_Hours_Start + 1) * UI_HOUR_HEIGHT + 10);
            lv_obj_set_pos(day_line, UI_TIME_LABEL_WIDTH + 15 + (i + 1) * UI_DAY_WIDTH, 20);
            lv_obj_set_style_bg_color(day_line, lv_color_hex(0xC0C0C0), 0);
            lv_obj_set_style_border_width(day_line, 0, 0);
            lv_obj_clear_flag(day_line, LV_OBJ_FLAG_SCROLLABLE);
        }
    }

    /* Create horizontal lines for each hour */
    for (uint8_t i = 0; i <= (_ui_Hours_End - _ui_Hours_Start); i++) {
        lv_obj_t *hour_line = lv_obj_create(calendar_container);
        lv_obj_set_size(hour_line, UI_DAY_WIDTH * 2 + 10, 1);
        lv_obj_set_pos(hour_line, UI_TIME_LABEL_WIDTH + 10, 20 + i * UI_HOUR_HEIGHT);
        lv_obj_set_style_bg_color(hour_line, lv_color_hex(0xC0C0C0), 0);
        lv_obj_set_style_border_width(hour_line, 0, 0);
        lv_obj_clear_flag(hour_line, LV_OBJ_FLAG_SCROLLABLE);
    }
...
void UI_Dashboard_Update_Time(struct tm *timeinfo)
{
    ...
    if ((header_label == NULL) || (timeinfo == NULL)) {
        return;
    }

    snprintf(time_str, sizeof(time_str), "Dashboard - %s %02d, %d %02d:%02d %s",
    ...
    lv_label_set_text(header_label, time_str);

    /* Update day labels */
    if ((day_labels[0] != NULL) && (day_labels[1] != NULL)) {
        char day_str[50];
        ...
        snprintf(day_str, sizeof(day_str), "%s, %s %02d",
                 ...
                 months[timeinfo->tm_mon],
                 timeinfo->tm_mday);
        lv_label_set_text(day_labels[0], day_str);
        ...
        snprintf(day_str, sizeof(day_str), "%s, %s %02d",
                 weekdays[tomorrow.tm_wday],
                 months[tomorrow.tm_mon],
                 tomorrow.tm_mday);
        lv_label_set_text(day_labels[1], day_str);
    }
}
...
void UI_Dashboard_Update_Battery(uint8_t Percentage)
{
    ...
    snprintf(battery_str, sizeof(battery_str), "%s %d%%", symbol, Percentage);
                           ...
                           const char *location, const char *description)
{
    ...
    /* Determine day column (0 = today, 1 = tomorrow) */
    day_col = (strcmp(day, "today") == 0) ? 0 : 1;
    ...
    x_pos = UI_TIME_LABEL_WIDTH + 15 + day_col * UI_DAY_WIDTH;
    y_pos = 25 + (hour - _ui_Hours_Start) * UI_HOUR_HEIGHT;
    ...
    event_data->start_time = start_time ? strdup(start_time) : NULL;
    event_data->end_time = end_time ? strdup(end_time) : NULL;
    ...
    lv_obj_set_size(event_box, UI_DAY_WIDTH - 15, UI_HOUR_HEIGHT - 5);
    lv_obj_set_pos(event_box, x_pos, y_pos);
    lv_obj_set_style_bg_color(event_box, lv_color_hex(0xD0D0D0), 0);
    lv_obj_set_style_border_width(event_box, 1, 0);
    lv_obj_set_style_border_color(event_box, lv_color_hex(0x808080), 0);
    lv_obj_set_style_radius(event_box, 5, 0);
    lv_obj_set_style_pad_all(event_box, 5, 0);
    lv_obj_clear_flag(event_box, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_label_set_text(event_label, title);
    ...
    lv_obj_set_width(event_label, UI_DAY_WIDTH - 30);
    lv_obj_align(event_label, LV_ALIGN_TOP_LEFT, 0, 0);

    ESP_LOGI(TAG, "Added event '%s' with details", title);
}
